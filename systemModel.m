function [rate, power, numOfRFchains] = systemModel(Nt, Nr, Lt, K, noise_variance, Lmax)
    ...
    H = mmWaveChannel(Nr, Nt, K, nodeLocations, Lmax);
    U = zeros(Nr, Nr, K);
    DD = zeros(Nr, Nt, K);
    V = zeros(Nt, Nt, K);
    ...
    for k=1:K
      eta_k = 0;
      for l=[1:k-1 k+1:K]
        eta_k = eta_k + norm(rho*U(:, 1, k)'*H(:,:,l)*V(:, 1, l))^2;
      end
    ...
    for k=1:K
      eta_k = 0;
      for l=[1:k-1 k+1:K]
        eta_k = eta_k + norm(U(:, 1, k)'*H(:,:,l)*F_RF(:,:,l))^2;
      end
    ...
    for lt=1:Lt
        ...
        for k=1:K
          eta_k = 0;
          for l=[1:k-1 k+1:K]
            eta_k = eta_k + norm(U(:, 1, k)'*H(:,:,l)*D(:, 1:Lt)*S_rf_min)^2;
          end
    ...
    if(Lt<=12)
        ...
        for lt=1:size(binary_combinations, 1)
           ...
           for k=1:K
             eta_k = 0;
             for l=[1:k-1 k+1:K]
               eta_k = eta_k + norm(U(:, 1, k)'*H(:,:,l)*D(:,1:Lt)*S_bf)^2;
             end
...
diag(S_bf_opt)'
    ...
    numOfRFchains = nnz(diag(S));
    rate(:, 6) = 0;
    for k=1:K
      eta_k = 0;
      for l=[1:k-1 k+1:K]
        eta_k = eta_k + norm(U(:, 1, k)'*H(:,:,l)*D*S)^2;
      end
