...
for lt_index = 1:length(lt_range)
    ...
    for r=1:totalMCrealizations
        [rate(r, lt_index, :), power(r, lt_index, :)] = systemModel(Nt, Nr, Lt, K, noise_variance);
    end
end
...
savefig(['./results/ee_lt_two_cols',num2str(Nt),'_',num2str(Lt),'_',num2str(Nr),'.fig'])
saveas(gcf,['./results/ee_lt_two_cols',num2str(Nt),'_',num2str(Lt),'_',num2str(Nr),'.eps'],'epsc')
